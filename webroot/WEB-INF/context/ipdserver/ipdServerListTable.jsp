...
<script type="text/javascript" src="${ctx}/scripts/server/ipdServer.js"></script>
					...
					<c:if test="${pager ne null}">
						<c:forEach items="${pager.list}" var="data" varStatus="status">
							<tr <c:choose>
								...
								<td  name = "accList"align="center">${data.localip}</td>
								<td name = "accList" align="center">${data.port}</td>
								<td name = "accList" align="center">${data.http}</td>
								<c:choose>
									<c:when test="${data.isMain == 1}">
