...
<body>
	...
	<div class="table_div">
		<form action="saveOrUpdate.action" method="post" onsubmit="return doSubmit();">
			<input type="hidden" name="id" value="${accountServer.id}" />
										...
										<table style="font-size: 15px;">
											...
											<tr>
												...
												<td class="text"><input type="text" name="name"
													value="${accountServer.name}" size="30" /></td>
											</tr>
											<tr>
												...
												<td class="text">
													<select id="gameId" name="gameId">
														...
														<c:forEach items="${game}" var="game">
															<option value="${game.id}" <c:if test="${accountServer.gameId == game.id}">selected="selected"</c:if>>${game.gameName}</option>
														</c:forEach>
													</select>
												</td>
											</tr>
											<tr>
												...
												<select id="serverId" name="serverId">
														...
														<c:forEach items="${server}" var="server">
															<option value="${server.id}" <c:if test="${accountServer.serverId == server.id}">selected="selected"</c:if>>${server.serverName}</option>
														</c:forEach>
												</select>
												</td>
											</tr>
											<tr>
												...
												<td class="text">
													<c:choose>
														<c:when test="${accountServer.id > 0}">
															<input type="text" name="modelPath" value="${accountServer.modelPath}" size="30" />
														</c:when>
														<c:otherwise>
															<select id="modelPath" name="modelPath">
																<option value="">请选择模板路径</option>
																<c:forEach items="${model}" var="model">
																	<option value="${model.accountPath}">${model.name}</option>
																</c:forEach>
															</select>
														</c:otherwise>
													</c:choose>
											...
											<tr>
												...
												<td class="text">
													<c:choose>
														<c:when test="${accountServer ne null }">
															<input type="text" name="updatePath" value="${accountServer.updatePath}" size="30" />
														</c:when>
														<c:otherwise>
															<input type="text" name="updatePath" value="/data/apps/model/update/account" size="30" />
														</c:otherwise>
													</c:choose>
												</td>
											</tr>
											...
											<tr>
												...
												<td class="text">
													<select id="state" name="state">
														...
														<option value="0" <c:if test="${accountServer.state == 0}">selected="selected"</c:if>>已关闭</option>
														<option value="1" <c:if test="${accountServer.state == 1}">selected="selected"</c:if>>已开启</option>
													</select>
												</td>
											</tr>
											<tr>
												...
												<td class="text">
													<select id="isDeploy" name="isDeploy">
														<option value="">请选择</option>
														<option value="0" <c:if test="${accountServer.isDeploy == 0}">selected="selected"</c:if>>未部署</option>
														<option value="1" <c:if test="${accountServer.isDeploy == 1}">selected="selected"</c:if>>已部署</option>
													</select>
												</td>
											</tr>
											<tr>
