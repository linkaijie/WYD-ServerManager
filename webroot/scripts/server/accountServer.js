function start(id){
	...
	function input(id) {
		var url = "accountserver/accountServerInput.action";
		if (id) {
			url += "?accountId=" + id;
		}
	...
	function batchStart() {
		...
		//没有选择提示错误
		if (data == "") {
			alert('请选择要启动的Account服务!');
			return;
		}
		if (data != "") {
			if (!confirm('您确定要启动当前选中Account服务吗?')) {
				return;
			}
		} else {
			return;
		...
		$.post(ctx + "/accountserver/batchStartAccount.action", {
	...
	function batchStop() {
		...
		//没有选择提示错误
		if (data == "") {
			alert('请选择要关闭的Account服务!');
			return;
		}
		if (data != "") {
			if (!confirm('您确定要关闭当前选中的Account服务吗?')) {
				return;
			}
		} else {
			return;
		...
		$.post(ctx + "/accountserver/batchCloseAccount.action", {
			accountIds : subString(data)
		}, function(msg) {
			if (msg == true) {
				//lhgdialog.opendlg("ACCOUNT日志",ctx+"/accountserver/batchCloseAccount.action",1020,500,true,true);
