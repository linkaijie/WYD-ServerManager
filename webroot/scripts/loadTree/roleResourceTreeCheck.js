...
function loadResourceTree(){
	...
	$.ajax({
		   type: "POST",
		   url: ctx+"/resource/loadResourceTree.action?paramId="+resourceId+"&node=",
		   success: function(msg){
			    zNodes = eval(msg);
			  	treeObj = $.fn.zTree.init($("#resourceTree"), setting, zNodes);
				treeObj.expandNode(treeObj.getNodes()[0], true, true, true);
				treeObj.setting.async.url = ctx+"/resource/loadResourceTree.action?paramId="+resourceId+"&node="+treeObj.getNodes()[0].id;
...
function getUrl(treeId, treeNode) {
	var resourceId=$("#resourceId").val();
	return ctx+"/resource/loadResourceTree.action?paramId="+resourceId+"&node="+treeNode.id;
}
