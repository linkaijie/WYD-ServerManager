...
function loadResourceTree(){
	...
	$.ajax({
		   type: "POST",
		   url: ctx+"/role/loadUserTree.action?node="+roleId,
		   success: function(msg){
			    zNodes = eval(msg);
			  	treeObj = $.fn.zTree.init($("#resourceTree"), setting, zNodes);
				treeObj.expandNode(treeObj.getNodes()[0], true, true, true);
				treeObj.setting.async.url = ctx+"/role/loadUserTree.action?node="+roleId;
...
function getUrl(treeId, treeNode) {
	var roleId=$("#roleId").val();
	return ctx+"/role/loadUserTree.action?node="+roleId;
}
