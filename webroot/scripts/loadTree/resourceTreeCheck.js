...
function loadResourceTree(){
	...
	$.ajax({
		   type: "POST",
		   url: ctx+"/role/loadAllTree.action?paramId="+roleId+"&node=",
		   success: function(msg){
			    zNodes = eval(msg);
			  	treeObj = $.fn.zTree.init($("#resourceTree"), setting, zNodes);
				//treeObj.expandNode(treeObj.getNodes()[0], true, true, true);
				//treeObj.setting.async.url = ctx+"/role/loadAllTree.action?paramId="+roleId+"&node="+treeObj.getNodes()[0].id;
...
function getUrl(treeId, treeNode) {
	var roleId=$("#roleId").val();
	return ctx+"/role/loadTree.action?paramId="+roleId+"&node="+treeNode.id;
}
